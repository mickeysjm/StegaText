...
def encode_patient_huffman(model, enc, message, context, bits_per_word, delta=1.0, finish_sent=False, device='cuda'):
    length = len(message)
    ...
    with torch.no_grad():
        ...
        while i < length or (finish_sent and not sent_finish):
            ...
            # Construct Huffman Tree on all tokens
            ...
            if i >= length:
                ...
                q = torch.exp(logq)
                cur_kl = kl(q, logq, log_probs)

                # Determine whether we want to use the Huffman tree or not
                if cur_kl < delta:  # use Huffman tree
...
def decode_patient_huffman(model, enc, text, context, bits_per_word, device='cuda'):
